...
SDF_Ground :: struct {
	triangles: []SDF_Ground_Triangle } // Unioned.
...
sdf_ground_triangle :: proc(p: [3]f32, ground_triangle: SDF_Ground_Triangle) -> Distance {
	a:    [3]f32
	b:    [3]f32
	c:    [3]f32
	n:    [3]f32
...
sdf_ground :: proc(p: [3]f32, ground: SDF_Ground) -> Distance {
	d: Distance

	d = SDF_CLEAR
	for _, i in ground.triangles do d = sdf_union(d, sdf_ground_triangle(p, ground.triangles[i]))
	return d }


sdf_mesh :: proc(p: [3]f32, mesh: SDF_Mesh) -> Distance {
	d: Distance
...
sdf_surface :: proc(p: [3]f32, surface: SDF_Surface) -> Distance {
	...
	case SDF_Ground_Triangle: return sdf_ground_triangle(p, variant)
	case SDF_Ground:          return sdf_ground(p, variant)
	case SDF_Mesh:            return sdf_mesh(p, variant) }
	return SDF_CLEAR }
