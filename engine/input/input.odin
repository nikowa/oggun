...
@(tag="job") input_tick :: proc(data_ptr: rawptr) {
	// fmt.println("Input Tick")
	data := cast(^Input_Tick_Data)data_ptr
	defer free(data)
	using data
	...
	// glfw.WaitEvents()
	glfw.PollEvents()
	input.keys_switched = input.keys_pressed ~ input.old_keys_pressed
	input.old_keys_pressed = input.keys_pressed }
...
key_callback :: proc "c" (window: glfw.WindowHandle, key, scancode, action, mods: i32) {
	...
	if key in input.keymap {
		switch(action) {
		case glfw.PRESS:   input.keys_pressed += { input.keymap[key] }
		case glfw.RELEASE: input.keys_pressed -= { input.keymap[key] } } } }
...
mouse_button_callback :: proc "c" (window: glfw.WindowHandle, button, action, mods: i32) {
	...
	case glfw.MOUSE_BUTTON_LEFT:
		if action == glfw.PRESS {
			input.mouse_pressed += { Mouse_Button.MOUSE_LEFT } }
		if action == glfw.RELEASE {
			input.mouse_pressed -= { Mouse_Button.MOUSE_LEFT } }
	case glfw.MOUSE_BUTTON_RIGHT:
		if action == glfw.PRESS {
			input.mouse_pressed += { Mouse_Button.MOUSE_RIGHT } }
		if action == glfw.RELEASE {
			input.mouse_pressed -= { Mouse_Button.MOUSE_RIGHT } } }
	if action == glfw.PRESS || action == glfw.RELEASE {
		input.mouse_switched = input.mouse_pressed ~ input.old_mouse_pressed
		input.old_mouse_pressed = input.mouse_pressed } }
...
mouse_was_pressed :: proc(input: ^Input, button: Mouse_Button) -> bool {
	return (button in input.mouse_pressed) && (button in input.mouse_switched) }


mouse_was_released :: proc(input: ^Input, button: Mouse_Button) -> bool {
	return (button in input.mouse_pressed == false) && (button in input.mouse_switched) }


key_was_pressed :: proc(input: ^Input, key: Key) -> bool {
	return (key in input.keys_pressed) && (key in input.keys_switched) }


key_was_released :: proc(input: ^Input, key: Key) -> bool {
	return (key in input.keys_pressed == false) && (key in input.keys_switched) }


input_init :: proc(input: ^Input, window: glfw.WindowHandle) {
