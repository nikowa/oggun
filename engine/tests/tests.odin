...
database_test :: proc(t_context: ^tst.T) {
	...
	entries: [2]^db.Entry
	database_0 := db.make_database({ "Data.bin", "data" }, context.temp_allocator)
	...
	for test_image, i in test_images {
		...
		assert(err == nil)
		bytes: []u8 = img.pixels.buf[:]
		url: string = db.url_join({ "image", sp.name(test_image, true, context.temp_allocator) }, context.temp_allocator)
