...
@(tag="job") camera_tick :: proc(data_ptr: rawptr) {
	...
	case .GAME:
		...
		case .FREE:
			...
			if .W in input.keys_pressed do forward_delta  += frame_time * camera.speed
			if .S in input.keys_pressed do forward_delta  -= frame_time * camera.speed
			if .D in input.keys_pressed do sideward_delta += frame_time * camera.speed
			if .A in input.keys_pressed do sideward_delta -= frame_time * camera.speed
			if .E in input.keys_pressed do upward_delta   += frame_time * camera.speed
			if .Q in input.keys_pressed do upward_delta   -= frame_time * camera.speed
