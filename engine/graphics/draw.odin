...
draw_init :: proc(draw: ^Draw, cache: ^Cache, working_directory_path: string, settings: ^Graphics_Settings) {
	width:         i32
	height:        i32
	ok:            bool
	texture:       ^Texture
	...
	draw.window = glfw.CreateWindow(i32(draw.window_size.x), i32(draw.window_size.y), "The Blue Break", settings.fullscreen ? glfw.GetPrimaryMonitor() : nil, nil)
	assert(draw.window != nil)
	glfw.MakeContextCurrent(draw.window)
	glfw.SwapInterval(0)
	gl.load_up_to(4, 6, glfw.gl_set_proc_address)
	gl.DebugMessageCallback(error_callback, nil)
	glfw.FocusWindow(draw.window)
	...
	width, height = glfw.GetFramebufferSize(draw.window)
	draw.window_size = { cast(int)width, cast(int)height }
	gl.Viewport(0, 0, cast(i32)draw.window_size.x, cast(i32)draw.window_size.y)
	gl.GenVertexArrays(1, &draw.vertex_array)
	gl.BindVertexArray(draw.vertex_array)
	gl.GenBuffers(1, &draw.vertex_buffer)
...
init_shader_params :: proc($Type: typeid, shader: ^Type) {
	...
	for _, i in fields {
		...
		case runtime.Type_Info_Named:
			using_struct := v.base.variant.(runtime.Type_Info_Struct)
...
init_shader_from_disk :: proc(draw: ^Draw, working_directory_path: string, name: string, $Type: typeid, vert_name: string, frag_name: string) -> ^Type {
	shader:  ^Type
	err:     runtime.Allocator_Error
	ok:      bool
	err_loc: runtime.Source_Code_Location
	...
	_, err = append(&draw.shaders, &shader.shader)
	if err != .None { return nil }
	ok,err_loc = compile_shader(draw,working_directory_path,shader)
	if !ok { return nil }
	init_shader_params(Type, shader)
	return shader }


shaders_init :: proc(draw: ^Draw, working_directory_path: string) {
	draw.shaders = make([dynamic]^Shader, 0, 16)
	draw.texture_shader              = init_shader_from_disk(draw, working_directory_path, "texture",              Texture_Shader,              "vrect",    "ftexture")
	draw.buffer_shader               = init_shader_from_disk(draw, working_directory_path, "buffer",               Buffer_Shader,               "vfill",    "fbuffer")
	draw.upscale_pass1_shader        = init_shader_from_disk(draw, working_directory_path, "buffer",               Upscale_Pass1_Shader,        "vfill",    "fupscale-pass1")
	draw.upscale_pass2_shader        = init_shader_from_disk(draw, working_directory_path, "buffer",               Upscale_Pass2_Shader,        "vfill",    "fupscale-pass2")
	draw.blend_shader                = init_shader_from_disk(draw, working_directory_path, "blend",                Blend_Shader,                "vfill",    "fblend")
	draw.curvature_shader            = init_shader_from_disk(draw, working_directory_path, "curvature",            Curvature_Shader,            "vfill",    "fcurvature")
	draw.font_shader                 = init_shader_from_disk(draw, working_directory_path, "font",                 Font_Shader,                 "vfont",    "ffont")
	draw.rect_shader                 = init_shader_from_disk(draw, working_directory_path, "rect",                 Rect_Shader,                 "vrect",    "frect")
	draw.point_shader                = init_shader_from_disk(draw, working_directory_path, "point",                Point_Shader,                "vpoint",   "fpoint")
	draw.line_shader                 = init_shader_from_disk(draw, working_directory_path, "line",                 Line_Shader,                 "vline",    "fline")
	draw.physics_shader              = init_shader_from_disk(draw, working_directory_path, "physics",              Physics_Shader,              "vframe",   "fphysics")
	draw.model_shader                = init_shader_from_disk(draw, working_directory_path, "model",                Model_Shader,                "vmodel",   "fmodel")
	draw.panel_shader                = init_shader_from_disk(draw, working_directory_path, "panel",                Panel_Shader,                "vrect",    "fpanel")
	draw.water_effect_shader         = init_shader_from_disk(draw, working_directory_path, "effect-water",         Water_Effect_Shader,         "vframe",   "effect-water.f")
	draw.sdf_shader                  = init_shader_from_disk(draw, working_directory_path, "sdf",                  SDF_Shader,                  "vframe",   "fsdf")
	draw.chromatic_aberration_shader = init_shader_from_disk(draw, working_directory_path, "chromatic-aberration", Chromatic_Aberration_Shader, "vfill",    "fchromatic-aberration") }
...
print_glsl_error :: proc(message: string, message_type: gl.Shader_Type, shader: ^Shader, vert_string: string, frag_string: string) {
	...
	case: print_bad(" Shader compilation error:",shader.name,": ",message_type,": ",message,": ?",sep="") }
	...
	print_bad(" Shader linking error:", message_type, "/", shader.name, "(", line_n, ")", ": ", ":\n", message, ": \n", line, sep = "") }


compile_shader :: proc(draw: ^Draw, working_directory_path: string, shader: ^Shader, allocator := context.allocator) -> (ok: bool, err_loc: runtime.Source_Code_Location) {
	vert_source:          string
	frag_source:          string
	...
	if ((shader.vert_name != "") && (shader.frag_name != "")) {
		extended_vert_name = fmt.aprint(shader.vert_name, ".glsl", sep = "")
		path = filepath.join({ working_directory_path, SHADERS_PATH_RELATIVE, extended_vert_name })
		if ! os.exists(path) do panic(fmt.aprintfln("File %s does not exist.", path))
		vert_source = read_file(path)
		extended_frag_name = fmt.aprint(shader.frag_name, ".glsl", sep = "")
		path = filepath.join({ working_directory_path, SHADERS_PATH_RELATIVE, extended_frag_name })
		if ! os.exists(path) do panic(fmt.aprintfln("File %s does not exist.", path))
		frag_source = read_file(path) }
	...
	_, err = init_glsl_builder(&builder); if err != .None { return false, #location() }
	fmt.sbprintln(&builder.string_builder, GLSL_VERSION_STRING)
	ok, loc = preprocess_glsl(draw,working_directory_path, &builder,vert_source)
	vert_string = strings.clone(glsl_builder_to_string(&builder))
	temp_filepath = filepath.join({working_directory_path,TEMP_PATH_RELATIVE,extended_vert_name})
	file_handle, errno = open_or_make(temp_filepath)
	assert(errno == os.ERROR_NONE)
	os.write_string(file_handle, vert_string)
	os.close(file_handle)
	destroy_glsl_builder(&builder);
	_, err = init_glsl_builder(&builder); if err != nil { return false, #location() }
	fmt.sbprintln(&builder. string_builder, GLSL_VERSION_STRING)
	ok, loc = preprocess_glsl(draw, working_directory_path, &builder, frag_source)
	frag_string = strings.clone(glsl_builder_to_string(&builder))
	temp_filepath = filepath.join({ working_directory_path, TEMP_PATH_RELATIVE, extended_frag_name })
	file_handle, errno = open_or_make(temp_filepath)
	assert(errno == os.ERROR_NONE)
	os.write_string(file_handle, frag_string)
	os.close(file_handle)
	destroy_glsl_builder(&builder)
...
swap_buffers :: proc(draw: ^Draw) {
	glfw.SwapBuffers(draw.window) }
...
preprocess_glsl :: proc(draw: ^Draw, working_directory_path: string, builder: ^GLSL_Builder, source: string) -> (ok: bool, loc: runtime.Source_Code_Location) {
	lines:         []string
	line:          string
	fields:        []string
	open:          u8
	...
	for _line in lines {
		if strings.starts_with(_line,"//") do continue
		line = strings.trim_left_space(_line)
		switch {
		case strings.starts_with(_line,"    "), strings.starts_with(_line,"\t"):
			fmt.sbprintln(&builder.string_builder,_line)
		case strings.starts_with(line,"#include"):
			...
			if (open == '\"') && (close == '\"') {
				lib_path = filepath.join({ working_directory_path, SHADERS_PATH_RELATIVE, fmt.tprint(fields[1][1:len(fields[1]) - 1], sep = "") })
				incl_source, found = read_file(lib_path); assert(found, fmt.tprint("could not find", lib_path)) }
			else if (open == '<') && (close == '>') {
				lib_path = filepath.join({ working_directory_path, SHADERS_PATH_RELATIVE, "library", fmt.tprint(fields[1][1:len(fields[1]) - 1], sep = "") })
				incl_source, found = read_file(lib_path); assert(found, fmt.tprint("could not find", lib_path)) }
			else {
				return false, #location() }
			if slice.contains(builder.includes[:], incl_source) {
				continue }
			ok, loc = preprocess_glsl(draw, working_directory_path, builder, incl_source)
			assert(ok, loc = loc)
		case strings.starts_with(line, "#optimize"):
			fields = strings.fields(line); assert(len(fields) >= 2)
			if fields[2] != "uniform" { return false, #location() }
			if (fields[3] != "float") && (fields[3] != "vec3") { return false, #location() }
			open = fields[1][0]
			close = fields[1][len(fields[1]) - 1]
			if open == '<' && close == '>' {
				param = fields[1][1:len(fields[1]) - 1]
				if param == "make_const" {
					fmt.sbprintln(&builder.string_builder, strings.join(fields[2:], sep = " ")) }
				else {
					return false, #location() } }
			else {
				return false, #location() }
		case strings.starts_with(line, "uniform"):
		...
		case strings.starts_with(line, "#define"):
			fmt.sbprintln(&builder.string_builder, _line)
		case starts_with_any(line, data_types):
			already_defined :: proc(builder: ^GLSL_Builder, type: string, name: string) -> (yes: bool, previous_type: string) {
...
@(tag = "job") draw_tick :: proc(data_ptr: rawptr) {
	data:      ^Draw_Tick_Data
	flare_pos: [3]f32
	pos:       [2]f32
	...
	if glfw.WindowShouldClose(draw.window) do draw.window_closed = true
