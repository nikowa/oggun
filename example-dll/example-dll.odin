...
dev_tick :: proc(camera_node: ^scn.Camera_Node, time: f32) {
	...
	// rotate_matrix = la.matrix4_from_euler_angles(pitch, 0, yaw, .XYZ)
	// camera_node.node.rotate = la.quaternion_from_euler_angles_f32(0, 0, 0, .XYZ)
	// camera_node.node.translate = arm_length * (rotate_matrix * [4]f32{ 0, 0, 0, 1 }).xyz
	camera_node.node.translate = ([4]f32{ 0, 0, -100, 1 } * la.matrix4_from_euler_angles_f32(pitch, 0, yaw, .XYZ)).xyz
