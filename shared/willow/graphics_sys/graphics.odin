...
init :: proc(
	gx_mngr: ^Graphics_Context = nil,
	as_mngr: ^as.Asset_Manager = nil,
	in_mngr: ^input_sys.Input_Manager = nil,
...
// 	glfw.SetWindowFocusCallback(draw.window, focus_callback)
	glfw.SetKeyCallback(gx_mngr.window, key_callback)
...
// 	glfw.SetInputMode(draw.window, glfw.RAW_MOUSE_MOTION, 0)
	glfw.SetWindowUserPointer(gx_mngr.window, in_mngr)
...
// 	init_texture_from_data(draw, texture, filepath.join({ working_directory_path, IMAGES_PATH_RELATIVE }), texture.name)
// 	load_texture(draw, texture)
// 	font.name = str.clone(name_from_path(path))
...
tick :: proc(gx_mngr: ^Graphics_Context) {
	tick_begin(gx_mngr) }
...
// @(tag = "job")
tick_begin :: proc(gx_mngr: ^Graphics_Context) {
...
tick_end :: proc(gx_mngr: ^Graphics_Context) {
	set_depth_test(false)
	select_frame_buffer(gx_mngr, 0)
	if gx_mngr.buffer_shader.handle != 0 do render_render_buffer(gx_mngr, &gx_mngr.canvas_rb, 0)
...
key_callback :: proc "c" (window: glfw.WindowHandle, key, scancode, action, mods: i32) {
	context = runtime.default_context()
	im: ^input_sys.Input_Manager = cast(^input_sys.Input_Manager)glfw.GetWindowUserPointer(window)
	assert(im != nil)
	input_sys.trigger(im, cast(input_sys.Input)key, action == glfw.RELEASE ? .Release : .Press) }
