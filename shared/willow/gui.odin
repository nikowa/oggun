...
rect_margins :: proc(rect_in: Rect, margins: f32) -> (rect_out: Rect) {
	rect_out = rect_in
...
rect_split_ratio_h :: proc(rect_in: Rect, ratio: f32, margin: f32) -> (rect_left: Rect, rect_right: Rect) {
	rect_left  = rect_in
	rect_right = rect_in
...
rect_split_ratio_v :: proc(rect_in: Rect, ratio: f32, margin: f32) -> (rect_top: Rect, rect_bottom: Rect) {
	rect_top    = rect_in
	rect_bottom = rect_in
...
rect_slice_h_append :: proc(rect_in: Rect, size: f32, n_max: int, rects: ^[dynamic]Rect) {
...
rect_slice_h_make :: proc(rect_in: Rect, size: f32, n_max: int, allocator: runtime.Allocator) -> (rects_out: []Rect) {
...
rect_slice_v_append :: proc(rect_in: Rect, size: f32, n_max: int, rects: ^[dynamic]Rect) {
	slice_rects := rect_slice_v_make(rect_in, size, n_max, context.temp_allocator)
	for slice_rect in slice_rects do append(rects, slice_rect) }

rect_slice_v_make :: proc(rect_in: Rect, size: f32, n_max: int, allocator: runtime.Allocator) -> (rects_out: []Rect) {
	if n_max == 0 do return {}
...
rect_grid_make :: proc(rect_in: Rect, size: [2]int, allocator: runtime.Allocator) -> (rects_out: []Rect) {
...
rect_screen :: proc(graphics_manager: ^Graphics_Manager) -> Rect {
	return make_rect(0.0, 0.0, graphics_manager.active_resolution.x, graphics_manager.active_resolution.y) }

rect_extend :: proc(rect_in: Rect, left: f32 = 0, right: f32 = 0, bottom: f32 = 0, top: f32 = 0) -> (rect_out: Rect) {
	rect_out = rect_in
...
rect_mirror_x :: proc(rect_in: Rect, center: f32) -> (rect_out: Rect) {
	rect_out = rect_in
	delta: f32 = rect_in.pos.x - center
	rect_out.pos.x -= 2 * delta
	return rect_out }

rect_mirror_y :: proc(rect_in: Rect, center: f32) -> (rect_out: Rect) {
	rect_out = rect_in
...
rects_mirror_x_make :: proc(rects_in: []Rect, center: f32, allocator: runtime.Allocator) -> (rects_out: []Rect) {
	rects_out = make([]Rect, len(rects_in))
	for rect, i in rects_in do rects_out[i] = rect_mirror_x(rect, center)
	return rects_out }

rects_mirror_y_make :: proc(rects_in: []Rect, center: f32, allocator: runtime.Allocator) -> (rects_out: []Rect) {
	rects_out = make([]Rect, len(rects_in))
	for rect, i in rects_in do rects_out[i] = rect_mirror_y(rect, center)
	return rects_out }

rects_mirror_x_edit :: proc(rects: []Rect, center: f32) {
	for &rect, i in rects do rect = rect_mirror_x(rect, center) }

rects_mirror_y_edit :: proc(rects: []Rect, center: f32) {
	for &rect, i in rects do rect = rect_mirror_y(rect, center) }

rects_merge :: proc(rect_a: Rect, rect_b: Rect) -> (rect_out: Rect) {
...
rects_remove_range :: proc(rects: ^[dynamic]Rect, range: [2]int) {
	for i, j in range[0] ..< range[1] {
		ordered_remove(rects, i - j) } }

rects_merge_range :: proc(rects: ^[dynamic]Rect, range: [2]int) {
...
rects_merge_range_retaining :: proc(rects: ^[dynamic]Rect, range: [2]int) {
	rect_a := rects[range[0]]
	rect_b := rects[range[1] - 1]
	ordered_remove(rects, range.y - 1)
	rects[range.x] = rects_merge(rect_a, rect_b) }

rect_offset :: proc(rect_in: Rect, offset: [2]f32) -> (rect_out: Rect) {
	rect_out = rect_in
	rect_out.pos += offset
	return rect_out }
