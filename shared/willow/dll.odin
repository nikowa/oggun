...
make_dll :: proc($T: typeid, source_relpath: string) -> (dll_object: T, err: os.Error) where intrinsics.type_has_field(T, "base"), intrinsics.type_field_type(T, "base") == DLL {
	name, dir: string
	ok: bool
...
dll_was_modified :: proc(dll_object: ^$T) -> (was_modified: bool) where intrinsics.type_has_field(T, "base"), intrinsics.type_field_type(T, "base") == DLL {
	return file_was_modified(dll_object.base.source_relpath, &dll_object.base.modification_time) }

reload_dll :: proc(dll_object: ^$T) -> (err: os.Error) where intrinsics.type_has_field(T, "base"), intrinsics.type_field_type(T, "base") == DLL {
...
watch_dll :: proc(dll_object: ^$T) -> (ok: bool) where intrinsics.type_has_field(T, "base"), intrinsics.type_field_type(T, "base") == DLL {
	if dll_was_modified(dll_object) do return reload_dll(dll_object) == nil
	return false }

_load_dll :: proc(dll_object: ^$T) -> (ok: bool) where intrinsics.type_has_field(T, "base"), intrinsics.type_field_type(T, "base") == DLL {
