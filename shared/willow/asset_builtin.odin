...
init_string_asset :: proc(string_asset: ^String_Asset, config: Asset_Config) {
	config := config
	config.derived_type = String_Asset
	init_asset(String_Asset, &string_asset.asset, config) }

string_asset_command :: proc(asset: ^Asset, command: Asset_Command, watch: bool = false) -> (ok: bool) {
	...
	case .Import:
		...
		err: os.Error
		entry, existed := get_or_add_entry(asset.url)
		if ! existed || entry_was_modified(entry) {
			path := path_from_url(asset.url, context.temp_allocator)
			...
			add_or_update_entry(make_entry(asset.url, bytes, modification_time))
...
register_builtin_asset_kinds :: proc() {
	register_asset_kind(String_Asset, { command = string_asset_command }) }
