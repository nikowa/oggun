...
dr_button :: proc(rect: Rect, text: string, shape: GI_Button_Shape = .ROUNDED, appearance: GI_Appearance = .DEFAULT, disabled: bool = false, icon: GI_Icon = .None) {
	rect := rect
	rounding: f32 = 0.0
	switch shape {
...
// case .PRIMARY
	dr_rect(rect, fill_color = fill_color, stroke_color = stroke_color, stroke = stroke, rounding = rounding, depth = 0.9)
	// if hover do set_cursor(.Hand)
	// dr_icon :: proc(icon: GI_Icon, position: [2]f32, depth: f32 = 0.0, angle: f32 = 0.0) {
	if icon != .None {
	...
	dr_text_box(text, text_style, rect, h_align = .CENTER, v_align = .CENTER, depth = 0.1)
	// dr_rect_outline(rect, RED)
}

dr_icon :: proc(icon: GI_Icon, position: [2]f32, depth: f32 = 0.0, angle: f32 = 0.0) {
	// dr_rect_outline({ position, GI_ICON_SIZE }, RED)
	dr_text_symbol_rect(cast(u8)icon, { position, GI_ICON_SIZE }, depth, style = engine.gi_manager.icons_text_style, angle = angle) }
