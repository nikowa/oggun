...
Shader_Config :: struct #all_or_none {
	vert_url, frag_url: asset_manager.URL }

Shader_Asset :: struct {
	using asset: asset_manager.Asset,
	using shader_config: Shader_Config,
	vert_asset, frag_asset: asset_manager.String_Asset,
	handle: u32,
...
compile_shader :: proc(as_mngr: ^asset_manager.Asset_Manager, shader_asset: ^Shader_Asset, allocator := context.allocator) -> (err: os.Error) {
...
preprocess_glsl :: proc(database: ^asset_manager.Asset_Manager, working_directory_path: string, builder: ^GLSL_Builder, source: string) -> (loc: rt.Source_Code_Location, err: os.Error) {
	lines:         []string
	line:          string
	fields:        []string
	open:          u8
	...
	for line in lines {
		...
		if str.starts_with(line,"#include") {
			...
			if ((open == '\"') && (close == '\"')) || ((open == '<') && (close == '>')) {
				...
				path := asset_manager.relpath_to_source_path(database, relpath, context.temp_allocator)
...
shader_asset_command :: proc(as_mngr: ^asset_manager.Asset_Manager, asset: ^asset_manager.Asset, command: asset_manager.Asset_Command, watch: bool = false) -> (ok: bool) {
	...
	shader_asset := asset_manager.asset_object(asset, Shader_Asset, "asset")
	switch command {
	case .Validate:
		return true
	case .Query_Location:
