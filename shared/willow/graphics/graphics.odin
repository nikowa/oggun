...
// 	init_cubemap(&draw.cubemap, { 512, 512 })
	if as_mngr != nil {
		...
		assert(asset_manager.asset_command(as_mngr, Shader_Asset, &graphics_manager.buffer_shader.asset, .Import))
		...
		asset_manager.register_asset_kind(as_mngr, Image_Asset, { command = image_asset_command })
		asset_manager.register_asset_kind(as_mngr, Material_Asset, { command = image_asset_command }) }
	else {
...
// 	render_cubemap(draw, &draw.cubemap, camera.position)
	...
	graphics_manager.time = bs.read_stopwatch(&graphics_manager.stopwatch)
	select_render_buffer(graphics_manager, &graphics_manager.canvas_rb)
