...
image_modification_time :: proc(as_mngr: ^asset_manager.Asset_Manager, image: ^Image_Asset, location: asset_manager.Asset_Location_Field) -> (modification_time: tm.Time) {
	...
	case .Source_Directory:
		path := asset_manager.path_from_url(&as_mngr.database, image.url, context.temp_allocator)
		modification_time, _ := os.modification_time_by_path(path)
		return modification_time
	case .Database_File:
		path := asset_manager.relpath_to_path(as_mngr.database.relpath, context.temp_allocator)
		modification_time, _ := os.modification_time_by_path(path)
		return modification_time
	case .Database:
		entry, ok := asset_manager.get_entry(&as_mngr.database, image.url)
...
@private
image_asset_command :: proc(as_mngr: ^asset_manager.Asset_Manager, asset: ^asset_manager.Asset, command: asset_manager.Asset_Command, watch: bool = false) -> (ok: bool) {
	image := asset_manager.asset_object(asset, Image_Asset, "asset")
	switch command {
	case .Validate:
	case .Query_Location:
		path := asset_manager.path_from_url(&as_mngr.database, asset.url, context.temp_allocator)
		if os.exists(path) do asset.location += { .Source_Directory }
	case .Import:
		path: string
		err: os.Error
		if path, err = asset_manager.url_search_source(as_mngr, image.url, context.allocator); err != nil {
		...
		asset_manager.add_or_update_entry(as_mngr, asset_manager.make_entry(image.url, bytes, modification_time), true)
...
// import_or_retreive_image :: proc(database: ^asset_manager.Asset_Manager, url: asset_manager.URL, allocator: rt.Allocator) -> (image: Image_Asset, err: os.Error) {
// 	entry: ^asset_manager.Entry
// 	ok: bool
// 	path: string
// 	modification_time: t.Time
// 	bytes: []u8

// 	entry, ok = asset_manager.get_entry(database, url)
// 	if ok do if asset_manager.entry_was_modified(database, entry) || database.spec_modified do ok = false
// 	if ok do image = image_deserialize(entry.data, allocator) or_return
// 	else {
// 		log.infof("Reading image %s from source.", url)
// 		path = asset_manager.url_search_source(database, url, allocator) or_return
...
// 		asset_manager.add_or_update_entry(database, asset_manager.make_entry(url, bytes, modification_time), true) or_return }
