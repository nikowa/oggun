...
GI_Anim_Transition :: struct {
	value: f32,
	action_time: time.Duration,
	action_value: f32,
	direction: bool }

gi_anim_transition :: proc(range: [2]f32, initial_value: f32, speed: f32, initial_direction: bool, action: bool, location := #caller_location) -> (value: f32) {
	assert(range[1] > range[0])
	action := action
	state, ok := engine.gi_manager.anim_transitions[location]
	...
	engine.gi_manager.anim_transitions[location] = state
