...
bit_matrix_read :: proc { bit_matrix_read_bit, bit_matrix_read_interval }
...
bit_matrix_read_interval :: proc(bit_matrix: ^Bit_Matrix, first_index2: [2]u32, last_index2: [2]u32) -> Backing_Type {
	range: [2]u32 = { cast(u32)index2_to_index(bit_matrix.shape, first_index2), cast(u32)index2_to_index(bit_matrix.shape, last_index2) + 1 }
...
aprint_bit_matrix :: proc(bit_matrix: ^Bit_Matrix, allocator := context.allocator) -> string {
	...
	for i in 0 ..< bit_matrix.shape.x {
		for j in 0 ..< bit_matrix.shape.y do fmt.sbprint(&sb, bit_matrix_read_bit(bit_matrix, { i, j }))
		fmt.sbprintln(&sb) }
