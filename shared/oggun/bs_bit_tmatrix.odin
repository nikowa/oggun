...
clone_bit_tmatrix :: proc(bit_tmatrix: ^Bit_TMatrix, allocator := context.allocator) -> (Bit_TMatrix, runtime.Allocator_Error) #optional_allocator_error {
	backing, err := clone_bit_array(&bit_tmatrix.backing, allocator)
...
bit_tmatrix_fill_random :: proc(bit_tmatrix: ^Bit_TMatrix) {
	bit_array_fill_random(&bit_tmatrix.backing) }

bit_tmatrix_fill :: proc(bit_tmatrix: ^Bit_TMatrix, value: u8) {
	bit_array_fill(&bit_tmatrix.backing, value) }

bit_tmatrix_rank :: proc(bit_tmatrix: ^Bit_TMatrix, value: u8) -> (rank: u32) {
	rank += bit_array_rank(&bit_tmatrix.backing, value)
	if value == 0 do rank += triangular_size_complement(bit_tmatrix.shape)
	return rank }

bit_tmatrix_write :: proc(bit_tmatrix: ^Bit_TMatrix, index2: [2]u32, value: u8) {
	if index2.y > index2.x do return
	bit_array_write_bit(&bit_tmatrix.backing, index2_to_index_triangular(bit_tmatrix.shape, index2), value) }

bit_tmatrix_read :: proc(bit_tmatrix: ^Bit_TMatrix, index2: [2]u32) -> u8 {
	if index2.y > index2.x do return 0
	return bit_array_read_bit(&bit_tmatrix.backing, index2_to_index_triangular(bit_tmatrix.shape, index2)) }
...
aprint_bit_tmatrix :: proc(bit_tmatrix: ^Bit_TMatrix, allocator := context.allocator) -> string {
	...
	for i in 0 ..< bit_tmatrix.shape {
		for j in 0 ..< bit_tmatrix.shape do fmt.sbprint(&sb, bit_tmatrix_read(bit_tmatrix, { i, j }))
		fmt.sbprintln(&sb) }
