...
model_equiv :: proc(a: ^Model, b: ^Model) -> bool {
	return (a.url == b.url) &&
		slice.equal(a.positions, b.positions) &&
		slice.equal(a.normals, b.normals) &&
		slice.equal(a.texcoords, b.texcoords) &&
		slice.equal(a.lightmap_texcoords, b.lightmap_texcoords) }
...
model_serialize :: proc(
    model: ^Model,
    allocator: runtime.Allocator) -> (model_bytes: []u8, err: os.Error) {
	buffer: bytes.Buffer

	bytes.buffer_init_allocator(&buffer, 0, MODEL_MEMORY_CAP, context.temp_allocator)
	bytes.buffer_write_ptr(&buffer, model, size_of(model^)) or_return
	bytes.buffer_write_slice(&buffer, model.positions[:]) or_return
	bytes.buffer_write_slice(&buffer, model.normals[:]) or_return
	bytes.buffer_write_slice(&buffer, model.texcoords[:]) or_return
	bytes.buffer_write_slice(&buffer, model.lightmap_texcoords[:]) or_return
...
model_deserialize :: proc(
	...
	bytes.reader_init(&reader, model_bytes)
	bytes.reader_read_ptr(&reader, &model, size_of(model)) or_return
	...
	bytes.reader_read_slice(&reader, model.positions) or_return
	bytes.reader_read_slice(&reader, model.normals) or_return
	bytes.reader_read_slice(&reader, model.texcoords) or_return
	bytes.reader_read_slice(&reader, model.lightmap_texcoords) or_return
	return model, os.General_Error.None }
...
_load_model_gltf :: proc(
	...
	case:
		...
		return }
	translate_matrix := linalg.matrix4_translate_f32(node.translation)
	rotate_matrix := linalg.matrix4_from_quaternion_f32(cast(quaternion128)node.rotation)
	...
	// // base_gx_color_buffer_view := data.buffer_views[material_base_color_texture_image.buffer_view.?]
	// // base_gx_color_buffer := data.buffers[base_gx_color_buffer_view.buffer].uri.([]byte)
	// // base_gx_color_bytes := base_gx_color_buffer[base_gx_color_buffer_view.byte_offset:base_gx_color_buffer_view.byte_offset+base_gx_color_buffer_view.byte_length]
	// // base_color_name := fmt.aprintf("%s-base-color", material_name)
	// // init_texture_from_png(draw, &material.base_color_texture, base_color_name, base_gx_color_bytes)
...
// render_model_uv :: proc(draw: ^Draw, model: ^Model, rect: Rect = { position = { 0, 0 }, size = { 256, 256 } }) {
// 	// (TODO):
