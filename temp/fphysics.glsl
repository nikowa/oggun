...
point.x+NORMAL_HALF_DELTA,point.yz),displaced).x-
		csdf_scene(vec3(point.x-NORMAL_HALF_DELTA,point.yz),displaced).x,
		csdf_scene(vec3(point.x,point.y+NORMAL_HALF_DELTA,point.z),displaced).x-
		csdf_scene(vec3(point.x,point.y-NORMAL_HALF_DELTA,point.z),displaced).x,
		csdf_scene(vec3(point.xy,point.z+NORMAL_HALF_DELTA),displaced).x-
...
_uv=vec2((vec_angle(p.xy)+PI)/TWO_PI,0.5*(p.z/h)+0.5);
...
void main(void) {
	d_surf=csd_distance(csdf_scene(surf_position,false));
	d_surf_displaced=d_surf_displaced(csdf_scene(surf_position,true));
	d_surfer=csd_distance(csdf_scene(surfer_position,true));
	n_surf=csdf_normal(surf_position,false);
	n_surf_displaced=csdf_normal(surf_position,true); }
