...
layout(binding=5) uniform sampler2D dev_grid_sampler;
layout(binding=6) uniform sampler2D dev_oriented_grid_sampler;
in vec2 tex_coord;
uniform float time;
uniform vec2 res;
...
csd_t csdf_scene(vec3 p,bool displaced) {
	...
	const float MAX_HEIGHT=4;
	const float MIN_LENGTH=0;
	const float MAX_LENGTH=12;
	sd_t sd=SDF_CLEAR;
	...
	// csd=csdf_union(csd,csdf_sphere(c-vec3(1,0,0.5),0.05,SKIN));
	#ifndef PHYSICS
		vec3 camera_direction=vec3(0,1,0);
		vec3 camera_side_direction=vec3(1,0,0);
		vec3 surfer_direction=normalize(vec3(camera_direction.xy,0));
		vec3 left_hand=surfer_position-camera_side_direction*0.3+surfer_direction*0.2;
		vec3 right_hand=surfer_position+camera_side_direction*0.3+surfer_direction*0.2;
		const vec3 up_direction=vec3(0,0,1);
		const float REACH=0.3;
		const vec3 hand_size=vec3(0.05,0.05,0.025);
		csd=csdf_union(csd,csdf_sphere(p-surfer_position,0.1,SKIN));
		csd=csdf_union(csd,csdf_directed_box(
			p-left_hand-float(swimming)*REACH*(vec3(mat2_rotate(-0.2)*(surfer_direction.xy),0)*cos(8*time)-up_direction*sin(8*time)),
			hand_size,
			surfer_direction,
			up_direction,
			SKIN));
		csd=csdf_union(csd,csdf_directed_box(
			p-right_hand-float(swimming)*REACH*(vec3(mat2_rotate(0.2)*(surfer_direction.xy),0)*cos(8*time+PI)-up_direction*sin(8*time+PI)),
			hand_size,
			surfer_direction,
			up_direction,
			SKIN));
		...
		// SURF //
		c=p-surf_position;
		csd_t surf=csdf_surf(c.x*surf_direction+c.z*surf_up_direction-c.y*surf_side_direction);
		// surfer_direction
		csd=csdf_union(csd,surf);
	#endif
...
color_t shade_surface_by_cone(vec3 surface,vec3 cone,float near_clip,float far_clip) {
	...
	if (csd.x<EPSILON) {
		float depth=distance(light_origin_point,vec3(camera_position.x,camera_position.yz))/camera_far_clip;
		gl_FragDepth=depth;
		// if (depth < 0.05) {
...
void main(void) {
	...
	color.xyz=shade_surface_by_cone(camera_position,receptor_vector,0,camera_far_clip); }


_cone(camera_position,receptor_vector,0,eye_limit); }
