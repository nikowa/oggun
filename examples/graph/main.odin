...
entry_point :: proc(thread_data: ^willow.Thread_Data) {
	...
	bg_color := engine.tgui_manager.theme[TGUI_Theme_Key.NEUTRAL_BACKGROUND_3][0]
	fg_color := engine.tgui_manager.theme[TGUI_Theme_Key.NEUTRAL_FOREGROUND_1][0]
	stroke_color := engine.tgui_manager.theme[TGUI_Theme_Key.NEUTRAL_STROKE_1][0]
	...
	for engine_running() {
		...
		if engine_tick() {
			...
			arrow := head
			// char: u8 = cast(u8)TGUI_Icon.Save
			char: u8 = cast(u8)time
			arrow_rect: Rect = { { 0, 200 }, { 24, 24 } }
			draw_text_symbol_rect(char, arrow_rect, 0.5, style = engine.tgui_manager.icons_text_style, angle = 0 * time)
