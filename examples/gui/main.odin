...
entry_point :: proc(thread_data: ^willow.Thread_Data) {
	...
	for engine_running() do if engine_tick() {
		...
		gx_depth_push_inc(0.01)
		for rect in rects do dr_rect(rect_margins(rect, Interval(8)), FILL_2, STROKE, stroke = 1)
		...
		gx_depth_push_inc(0.01)
		for rect in rects_0 do dr_rect(rect_margins(rect, Interval(8)), FILL_1, STROKE, stroke = 1)
