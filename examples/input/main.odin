...
make_rects :: proc(keyboard_rect: rect.Rect, allocator: runtime.Allocator) -> []rect.Rect {
	...
	rect_group_0, rect_group := gui.rect_split_ratio_v(keyboard_rect, 0.187, 20)
	DIV_H1 :: 0.672
	DIV_H2 :: 0.434
	rect_group_1, _rect := gui.rect_split_ratio_h(rect_group, DIV_H1, 10)
	rect_group_2, rect_group_3 := gui.rect_split_ratio_h(_rect, DIV_H2, 10)

	// Esc
	rect_group_0_0: rect.Rect
	rect_group_0_0, _rect = gui.rect_split_ratio_h(rect_group_0, 0.065, 40)
	append(&rects, rect_group_0_0)

	// F1 - F4
	rect_group_0_1: rect.Rect
	rect_group_0_1, _rect = gui.rect_split_ratio_h(_rect, 0.207, 20)
	gui.rect_grid(rect_group_0_1, { 4, 1 }, &rects)

	// F5 - F8
	rect_group_0_2: rect.Rect
	rect_group_0_2, _rect = gui.rect_split_ratio_h(_rect, 0.266, 20)
	gui.rect_grid(rect_group_0_2, { 4, 1 }, &rects)

	// F9 - F12
	rect_group_0_3: rect.Rect
	rect_group_0_3, _rect = gui.rect_split_ratio_h(_rect, 0.36, 10)
	gui.rect_grid(rect_group_0_3, { 4, 1 }, &rects)

	// PrtScr ScrLk Pause
	rect_group_0_4: rect.Rect
	rect_group_0_4, _rect = gui.rect_split_ratio_h(_rect, 0.43, 10)
	...
	_rect, rect_group_1_1 = gui.rect_split_ratio_h(rect_group_1_1, 0.099, 0)
	append(&rects, _rect)
	gui.rect_slice_h_append(rect_group_1_1, KEY_WIDTH, 13, &rects)

	// CapsLock - Enter
	rect_group_1_2 := lines[2]
	_rect, rect_group_1_2 = gui.rect_split_ratio_h(rect_group_1_2, 0.116, 0)
	append(&rects, _rect)
	gui.rect_slice_h_append(rect_group_1_2, KEY_WIDTH, 12, &rects)

	// LShift - RShift
	rect_group_1_3 := lines[1]
	_rect, rect_group_1_3 = gui.rect_split_ratio_h(rect_group_1_3, 0.150, 0)
	append(&rects, _rect)
...
entry_point :: proc(thread_data: ^base.Thread_Data) {
	...
	graphics.init(
		...
		as_mngr = &asset_man,
		graphics_config = { window_manager = &window_manager, clear_color = graphics.BLACK })

	graphics.bitmap_font_init(&asset_man, &font, { name = "terminus", default_bearing = 0, default_advance = 0 })
	image: graphics.Image_Asset
	graphics.init_image(&asset_man, &image, { url = "image:kitten-2.png" })
